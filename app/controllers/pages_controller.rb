class PagesController < ApplicationController
  ...
  def bus_board
    ...
    stops = get_stops_from_postcode(postcode_details, 2)

    if stops.none?(&:has_buses?)
    ...
    @stops = stops.select { |stop| stop.has_buses? }
  end

  private

  def get_stops_from_postcode(postcode_details, num_stops)
    transport_api = TransportApi.new
    stopcodes = transport_api.get_nearest_stopcodes(num_stops, postcode_details.latitude, postcode_details.longitude)
